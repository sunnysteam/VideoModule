...
public class NetworkUtil {
    ...
    public static final int NET_NOT_PREPARE = 3; // Net no ready 网络没有准备好
     ...
     * @param callback
     */
    public static void getNetState(final Context context, final NetDetectorCallback callback) {
        executor.execute(new Runnable() {
            @Override
            public void run() {
                final int netState = getNetState(context);
                    ...
                    public void run() {
                        switch (netState) {
                            ...
                            case NET_CNNT_BAIDU_TIMEOUT:
                                callback.onDetectorFailed(netState);
                                break;
                            case NET_CNNT_BAIDU_OK:
                                callback.onDetectorOK();
                                break;
                        }
                    }
                });
            ...
            if (connectivity != null) {
                ...
                if (networkinfo != null) {
                    if (networkinfo.isAvailable() && networkinfo.isConnected()) {
                        return connectionNetwork() ? NET_CNNT_BAIDU_OK : NET_CNNT_BAIDU_TIMEOUT;
                    } else {
    ...
    static private boolean connectionNetwork() {
        ...
        try {
            httpUrl = (HttpURLConnection) new URL("https://www.baidu.com")
                    .openConnection();
